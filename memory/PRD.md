...
- **HospitalityOwnerDashboard.jsx** (cream `#FDF8F2` + amber `#E87722` + burgundy `#7C1D3F`) — full-width Food Safety Status Bar at top with 5 equipment cards (walk-in fridge / FOH display / freezer / bain-marie / dishwasher), dual horizontal score bars (WHS Safety + Food Safety), staff certifications grid (RSA / Food Handler / First Aid coloured dots per worker), Council Inspection burgundy card with checklist + Generate inspection pack CTA, today's cleaning, AI alerts.
- **TransportOwnerDashboard.jsx** (charcoal `#1C2526` + teal `#0DC4B5` + safety orange `#FF6B35`) — dark command-centre theme. Horizontal-scrolling Fleet Status Strip with vehicle cards. Three-column below: drivers compliance + dual mini-gauges (WHS / CoR) + 6-element CoR matrix (Fatigue / Speed / Mass-and-Dimension / Load Restraint / Vehicle Standards / Scheduling) + AI alerts.
- **HealthcareOwnerDashboard.jsx** (clinical white `#F4F8FB` + teal `#2196A6` + soft green `#4CAF8F`) — credential status panel dominant (4-column AHPRA/Screen/Vacc/First Aid table with "Fully current/Action required/Critical gap" status pill per row), 8-pill ACQSC Quality Standards grid, three horizontal score bars (WHS / Care Quality / Credential Compliance), AI alerts, WHS/Clinical tabs, full-width upcoming renewals at bottom.
- **RetailOwnerDashboard.jsx** (purple `#5B2D8E` + magenta `#E91E8C` + tint `#F9F5FF`) — energetic. Top band 3 large status tiles (Lone Worker / Inductions X/Y / Incidents Today), Workplace Safety Score with 4 sub-bars (WHS Documents / Inductions / Staff Credentials / Lone Worker Safety), today's roster + Quick Induct all bulk action, open hazards, 4 quick-action tiles (Quick Induct / Log Incident / Report Hazard / Lone Worker Check-In), credentials expiring.
- **Trades dashboard preserved unchanged** — owner+trades still routes to existing `OwnerDashboard`.
- **Industry-aware sidebar** — `DashboardLayout.jsx` reads `user.industry` and applies `NAV_LABELS_BY_INDUSTRY` (SWMS Library → Food Safety / Fleet & CoR / Care Quality / Inductions; Workers → Team Members / Drivers & Operators / Staff & Clinicians) + `APPS_NAV_BY_INDUSTRY` aliases (TradeInduct → VenueInduct / FleetInduct / ClinicInduct / StoreInduct; TradeCheck → VenueCheck / FleetCheck / ClinicCheck / StoreCheck) + `industryAccent` 4px left-border on active items (yellow/amber/teal/blue/purple per industry).
- **Testing** — Smoke screenshots confirm all 4 new dashboards render with completely distinct visual identities. Variant routing also re-verified: industry+role_variant combos correctly route to safety/supervisor/worker variants when both are explicitly set. Backend regression unchanged (37/37 pass).
