...
EXTRA_DOC_TYPES = {
    "trades": {
        "swms": {
            ...
            "ref_prefix": "SWMS", "category": "swms",
            "system_prompt": "You are a senior Australian WHS consultant specialising in trades and construction high-risk construction work. You draft Safe Work Method Statements that meet WHS Regulation 299–306 (or the equivalent Vic OHS Reg 5.1.16).",
            "sections": [
                "Project details (site, contractor, principal contractor, date prepared, review date)",
                "Description of the high-risk construction work",
                "Step-by-step work activities with associated hazards",
                "Risk assessment matrix (likelihood × consequence, residual after controls)",
                "Hierarchy of control measures for each hazard",
                "PPE requirements per task",
                "Plant, equipment, tools and substances used",
                "Permits required (hot work, confined space, working at heights, electrical)",
                "Training and competency requirements",
                "Emergency procedures (incident response, first aid, evacuation)",
                "Sign-on register — worker name, role, signature, date",
            ],
            "inputs": [
                {"key": "contractor_name", "label": "Contractor / company name", "type": "text", "required": True},
                {"key": "site_address", "label": "Site address", "type": "text", "required": True},
                {"key": "principal_contractor", "label": "Principal contractor", "type": "text"},
                {"key": "hrcw_categories", "label": "HRCW categories", "type": "textarea", "placeholder": "e.g. working at heights >2m, electrical work, confined space, demolition", "required": True},
                {"key": "trade", "label": "Trade", "type": "select", "options": ["electrical", "plumbing", "carpentry", "demolition", "scaffolding", "concreting", "roofing", "painting", "tiling", "hvac", "general"], "required": True},
                {"key": "state", "label": "State / Territory", "type": "select", "options": ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "NT", "ACT"], "required": True},
                {"key": "crew_size", "label": "Crew size", "type": "text"},
                {"key": "duration", "label": "Expected duration", "type": "text"},
            ],
        },
        "jsa": {
            "label": "Job Safety Analysis (JSA)",
            "ref_prefix": "JSA", "category": "swms",
            "system_prompt": "You are an Australian WHS consultant. You draft Job Safety Analyses for trades work that is below the HRCW threshold but still warrants step-by-step hazard analysis.",
            "sections": [
                "Job description", "Step-by-step task breakdown",
                "Hazards identified per step", "Risk rating before controls",
                "Control measures (hierarchy of control)", "Risk rating after controls",
                "PPE checklist", "Approval and sign-on",
            ],
            "inputs": [
                {"key": "job_name", "label": "Job / task name", "type": "text", "required": True},
                {"key": "location", "label": "Location", "type": "text"},
                {"key": "trade", "label": "Trade", "type": "select", "options": ["electrical", "plumbing", "carpentry", "concreting", "general"], "required": True},
        ...
        "toolbox_talk": {
            "label": "Toolbox Talk",
            "ref_prefix": "TBT", "category": "training",
            "system_prompt": "You are a site supervisor preparing a 10-minute toolbox talk for an Australian construction crew. Focus on practical, plain-language safety messages.",
            "sections": [
                "Topic of the day + why it matters today",
                "Key risks specific to current site conditions",
                "3–5 do's and don'ts",
                "Q&A prompts for the crew",
                "Sign-on register",
            ],
            "inputs": [
                {"key": "topic", "label": "Topic", "type": "text", "required": True, "placeholder": "e.g. working at heights / hot work / electrical isolation"},
                {"key": "trade", "label": "Trade", "type": "select", "options": ["electrical", "plumbing", "carpentry", "demolition", "scaffolding", "concreting", "roofing", "general"], "required": True},
                {"key": "site_conditions", "label": "Site conditions today", "type": "textarea", "placeholder": "weather, recent incidents, new equipment"},
            ],
        },
        "pre_start_checklist": {
            "label": "Pre-Start Site Checklist",
            "ref_prefix": "PSC", "category": "operations",
            "system_prompt": "You are an Australian site supervisor preparing a pre-start daily site verification checklist.",
            "sections": [
                "Site & weather check", "Plant & equipment inspection",
                "Worker fitness & inductions verified", "Permits in place",
                "First aid & emergency response readiness", "Sign-off",
            ],
            "inputs": [
                {"key": "site_name", "label": "Site name", "type": "text", "required": True},
                {"key": "trade", "label": "Primary trade", "type": "select", "options": ["electrical", "plumbing", "carpentry", "demolition", "scaffolding", "concreting", "general"], "required": True},
                {"key": "high_risk_today", "label": "Any high-risk work today?", "type": "textarea"},
            ],
        },
        "whs_policy": {
            "label": "WHS / OHS Policy",
            "ref_prefix": "WHSP", "category": "policy",
            "system_prompt": "You are an Australian WHS consultant drafting a Work Health & Safety Policy for a small-to-medium trades business. Reference the WHS Act 2011 (or Victorian OHS Act 2004).",
            "sections": [
                "Policy statement & commitment", "Scope and application",
                "Responsibilities — PCBU / officers / workers / sub-contractors",
                "Hazard identification & risk management",
                "Consultation, cooperation and coordination",
                "Training and competency", "Incident reporting & investigation",
                "Review and continuous improvement",
            ],
            "inputs": [
                {"key": "company_name", "label": "Company name", "type": "text", "required": True},
                {"key": "trade_specialisation", "label": "Trade specialisation", "type": "text", "required": True},
                {"key": "state", "label": "State", "type": "select", "options": ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "NT", "ACT"], "required": True},
                {"key": "worker_count", "label": "Worker count", "type": "text"},
            ],
        },
        "subcontractor_agreement": {
            "label": "Subcontractor Safety Agreement",
            "ref_prefix": "SCA", "category": "contracts",
            "system_prompt": "You are an Australian construction lawyer-WHS consultant. You draft a subcontractor agreement that imposes safety, induction and insurance obligations on the subcontractor.",
            "sections": [
                "Parties", "Scope of works", "Safety obligations of the subcontractor",
                "Required licences, qualifications and inductions",
                "Insurance requirements (public liability, workers comp)",
                "Incident reporting obligations", "Indemnity & limitation of liability",
                "Termination", "Signatures",
            ],
            "inputs": [
                {"key": "principal", "label": "Principal contractor name", "type": "text", "required": True},
                {"key": "subbie", "label": "Subcontractor name", "type": "text", "required": True},
        ...
        "site_emergency_plan": {
            "label": "Site Emergency Response Plan",
            "ref_prefix": "SERP", "category": "emergency",
            "system_prompt": "You are an Australian construction site safety planner. You draft Emergency Response Plans aligned with WHS Reg 43 (emergency plans).",
            "sections": [
                "Site overview & evacuation map", "Emergency types covered (fire, medical, structural, hazardous-substance, severe weather)",
                "Roles & responsibilities (warden, deputy, first-aider)",
                "Evacuation procedures with assembly points",
                "Notification flow chart (000 → principal contractor → regulator)",
                "Communications equipment", "Drill schedule & records",
            ],
            "inputs": [
                {"key": "site_name", "label": "Site name", "type": "text", "required": True},
                {"key": "site_address", "label": "Site address", "type": "text", "required": True},
                {"key": "crew_size", "label": "Crew size on a typical day", "type": "text"},
                {"key": "high_risk_present", "label": "High-risk activities on site", "type": "textarea", "placeholder": "e.g. working at heights, demolition, electrical, confined space"},
                {"key": "state", "label": "State", "type": "select", "options": ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "NT", "ACT"], "required": True},
            ],
        },
        "induction_program": {
            "label": "Site Induction Program",
            "ref_prefix": "IND", "category": "training",
            "system_prompt": "You are an Australian construction trainer who builds site induction programs.",
            "sections": [
                "Welcome & site overview", "Site rules & boundaries",
                "PPE requirements", "High-risk work and permit requirements",
                "Hazards specific to this site", "Emergency & evacuation procedures",
                "Reporting obligations (incidents, near-misses, hazards)",
                "Sign-on & competency check",
            ],
            "inputs": [
                {"key": "site_name", "label": "Site name", "type": "text", "required": True},
                {"key": "trade_focus", "label": "Trade focus", "type": "text"},
                {"key": "site_specific_hazards", "label": "Site-specific hazards", "type": "textarea"},
                {"key": "state", "label": "State", "type": "select", "options": ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "NT", "ACT"], "required": True},
            ],
        },
        "incident_investigation": {
            "label": "Incident Investigation Report",
            "ref_prefix": "INC", "category": "incidents",
            "system_prompt": "You are an Australian WHS auditor writing up an incident investigation report.",
            "sections": [
                "Incident summary (what, where, when, who)",
                "Immediate cause(s)", "Underlying / root cause analysis (5 Whys)",
                "Contributing factors", "Corrective and preventive actions with owners and due dates",
                "Witness statements", "Photos / sketches",
                "Regulator notification status", "Sign-off",
            ],
            "inputs": [
                {"key": "incident_summary", "label": "What happened (1–2 sentences)", "type": "textarea", "required": True},
                {"key": "location", "label": "Location on site", "type": "text"},
                {"key": "severity", "label": "Severity", "type": "select", "options": ["near miss", "minor", "moderate", "serious", "critical"], "required": True},
                {"key": "injured_role", "label": "Role of injured worker (if applicable)", "type": "text"},
                {"key": "state", "label": "State", "type": "select", "options": ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "NT", "ACT"], "required": True},
            ],
        },
    },
